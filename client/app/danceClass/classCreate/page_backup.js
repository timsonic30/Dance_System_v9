...
export default function ClassCreate() {
        ...
        if (value.enumValues && value.enumValues.length !== 0) {
          ...
          let selectTag = (
            <div key={key} className="flex items-center pb-2 pl-3 pr-3">
              <span className="text-gray-500 w-24" style={{ marginRight: '3rem' }}>{key}</span>
              <select
    ...
    setFormData((prevFormData) => {
      ...
      if (key === "startTime" && updatedFormData.endTime) {
        if (!validateTimeOrder(value, updatedFormData.endTime)) {
          console.error("Error: End time cannot be earlier than start time.");
          alert("結束時間不能早於開始時間！請檢查輸入。");
        }
      ...
      console.log("Form Data:", updatedFormData); // 調試輸出
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:3030/danceclass/classCreate", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="bg-white rounded-lg p-6 border-2">
      <h2 className="text-xl font-semibold mb-6" style={{ marginBottom: '5rem' }}>Class Create</h2>
